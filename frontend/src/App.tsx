...
import { createNewGame, mapTileColorToCardType, updateGameState } from './api/gameApi';
...
function App() {
    ...
    try {
      ...
      newGameState.cards.forEach(card => {
        tiles[card.word] = {
          ...
          color: card.type === 'yellow' ? 'orange' : 
                 card.type === 'green' ? 'green' :
                 card.type === 'assassin' ? 'red' : 'beige'
        };
      });
  ...
  const switchTurn = (newGameState: GameState) => {
    newGameState.current_turn = newGameState.current_turn === 'orange' ? 'green' : 'orange';
  ...
  const handleCardClick = async (index: number) => {
    ...
    if (clickedCard.type === 'assassin') {
      ...
      newGameState.winner = gameState.current_turn === 'orange' ? 'green' : 'orange';
      ...
      if (gameState.current_turn !== 'orange' || 
      ...
      if (gameState.current_turn !== 'green' || 
          newGameState.guesses_this_turn >= (gameState.current_hint?.number || 0)) {
        switchTurn(newGameState);
      }
    } else {
  ...
  const handleAIGuess = async (guesses: AIGuessResult[]) => {
    ...
    for (const guess of guesses) {
      const cardIndex = currentGameState.cards.findIndex(card => card.word === guess.word);
      ...
      if (cardIndex !== -1) {
        ...
        if (clickedCard.type === 'assassin') {
          ...
          currentGameState.winner = currentGameState.current_turn === 'orange' ? 'green' : 'orange';
        ...
        if (currentGameState.guesses_this_turn >= (currentGameState.current_hint?.number || 0)) {
          switchTurn(currentGameState);
          break;
        }
  ...
  const initializeGame = async () => {
    ...
    try {
      ...
      setError('An error occurred while setting up the game. Using local word list as fallback.');
